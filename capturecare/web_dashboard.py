...
def generate_video_token(patient_id):
    ...
    try:
        ...
        try:
            ...
            # Handle both authenticated and unauthenticated users (optional_login_required decorator)
            if current_user.is_authenticated:
        ...
        try:
            ...
            if 'company_assets' in inspector.get_table_names():
                ...
                required_columns = {
                    ...
                    'file_type': 'VARCHAR(200)',  # Increased from 50 to accommodate long MIME types
                    'file_size': 'INTEGER',
                    'link_url': 'TEXT',
                    'tags': 'VARCHAR(500)',
                ...
                # Also check and fix file_type column size (needs to be VARCHAR(200) for long MIME types)
                try:
                    with db.engine.connect() as conn:
                        if 'postgresql' in str(db.engine.url).lower():
                            conn.execute(text("ALTER TABLE company_assets ALTER COLUMN file_type TYPE VARCHAR(200)"))
                            conn.commit()
                            logger.info("✅ Increased file_type column size to VARCHAR(200)")
                        # SQLite doesn't support ALTER COLUMN, but new installs will have correct size
                except Exception as e:
                    if "already" in str(e).lower() or "same" in str(e).lower():
                        logger.info("⏭️  file_type column already correct size")
                    else:
                        logger.warning(f"Could not resize file_type column: {e}")
